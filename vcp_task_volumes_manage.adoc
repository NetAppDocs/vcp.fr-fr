...
Les systèmes d'exploitation diffèrent dans leur gestion des volumes clonés. ESXi traite un volume cloné comme une copie de volume ou un volume snapshot. Le volume sera un périphérique disponible pour créer un nouveau datastore. Pour plus d'informations sur le montage des volumes clonés et la gestion des LUN snapshot, consultez la documentation VMware concernant https://techdocs.broadcom.com/us/en/vmware-cis/vsphere/vsphere/7-0/vsphere-storage/working-with-datastores-in-vsphere-storage-environment/vsphere-vmfs-datastore-copies-and-datastore-resignaturing/resignature-a-vmfs-datastore-copy-in-the-vsphere-web-client.html["Montage d'une copie de datastore VMFS"^] et https://techdocs.broadcom.com/us/en/vmware-cis/vsphere/vsphere/7-0/vsphere-storage/working-with-datastores-in-vsphere-storage-environment/vsphere-vmfs-datastore-copies-and-datastore-resignaturing.html["Gérer les datastores VMFS en double"^].
